package saomath.checkusserver.health;
...
public class HealthController {
    ...
    @GetMapping("/health")
    public ResponseEntity<ResponseBase<Map<String, Object>>> healthCheck() {
        ...
        return ResponseEntity.ok(ResponseBase.success("서버가 정상적으로 동작 중입니다.", healthInfo));
    }

    @Operation(summary = "API 버전 확인", description = "현재 API 버전 정보를 반환합니다.")
    @GetMapping("/version")
    public ResponseEntity<ResponseBase<Map<String, String>>> getVersion() {
        Map<String, String> versionInfo = new HashMap<>();
        versionInfo.put("version", "1.0.0");
        versionInfo.put("build", "2024.12.20");
        versionInfo.put("api", "v1");
        
        return ResponseEntity.ok(ResponseBase.success(versionInfo));
    }
}
